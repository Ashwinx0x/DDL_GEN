  ...
  FOR pii_view IN (
    ...
    FOR pii_col IN (
      ...
      IF REGEXP_CONTAINS(view_select_part, r'\bt\.' || pii_col.PII_COLUMN_NAME || r'\b') THEN
        ...
        -- Build LEFT JOIN
        SET join_cond = join_cond || 'LEFT JOIN `cs-cdwp-data-pp2182.CDW_XREF_DATA.' || pii_col.XREF_TABLE_NAME || '` xr' || CAST(counter AS STRING) || ' \nON t.' || pii_col.PII_COLUMN_NAME || ' = xr' || CAST(counter AS STRING) || '.' || pii_col.XREF_JOIN_COLUMN_NAME || '\n';
    ...
    SET new_ddl = new_ddl || '\nFROM `cs-cdwp-data-pp2182.' || BASE_TBL_DB || '.' || BASE_TBL_NM || '` t \n';
